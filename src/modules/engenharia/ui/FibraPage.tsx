...
const FibraPage = () => {
  ...
  return (
    <div className="space-y-3">
      <div>
        <h2 className="text-xl font-display font-semibold">Fibra — Checklist Projeto Executivo</h2>
        <p className="text-xs text-muted-foreground">Padrões CLARO / SEINFRA-DNIT / COELBA por obra.</p>
      </div>

      <Card>
        <CardContent className="pt-4 flex flex-wrap items-center gap-2">
          <Button onClick={() => setOpenNew(true)}><Plus className="h-4 w-4 mr-1" />Novo checklist</Button>
                  ...
                  {filtered.map((c) => {
                    ...
                    return (
                      <TableRow key={c.id} className="cursor-pointer" onClick={() => setEditing(c)}>
                        ...
                        <TableCell>{c.responsavel_geral || "—"}</TableCell>
                        <TableCell><Badge variant="outline">{progressoPorPadrao(items, "CLARO")}%</Badge></TableCell>
                        <TableCell><Badge variant="outline">{progressoPorPadrao(items, "SEINFRA")}%</Badge></TableCell>
                        <TableCell><Badge variant="outline">{progressoPorPadrao(items, "COELBA")}%</Badge></TableCell>
