...
export const pixiCharacters = {
  private characters: Map<string, PIXI.Container> = new Map(),
  ...
  render(characters: PixelCharacter[]) {
    ...
    // Remove old ones
    for (const [id, sprite] of this.characters.entries()) {
      if (!activeIds.has(id)) {
    ...
    characters.forEach(char => {
      let charContainer = this.characters.get(char.user_id);
      
      if (!charContainer) {
        charContainer = this.createCharacterSprite(char);
        this.characters.set(char.user_id, charContainer);
  ...
  createCharacterSprite(char: PixelCharacter): PIXI.Container {
    const container = new PIXI.Container();
    container.label = `char-${char.user_id}`;
    
    // Base Graphics for the layered look (until we use actual textures/atlases)
    ...
    text.y = 25; // Below the avatar
    container.addChild(text);

    return container;
  },

  updateCharacterSprite(container: PIXI.Container, char: PixelCharacter) {
    ...
    // Smooth lerp or direct set (we'll do direct for now as React handles the state update)
    container.x = targetX;
    container.y = targetY;
    container.zIndex = char.position_y * 10 + 100;

    // Redraw graphics (this is expensive in PIXI if done every frame, but fine for now)
    graphics.clear();
    
    const colors = this.getCustomizationColors(customization);
    
    // Draw shadow
    graphics.ellipse(0, 0, 8 * SCALE, 2 * SCALE);
    graphics.fill({ color: 0x000000, alpha: 0.35 });

    // This is a simplified version of AvatarLayeredSprite for PIXI
    // To be truly performant, we should generate textures from the SVGs once
    ...
    // Scaling factor (original 16x24, center is x=8, y=24)
    const s = SCALE;
    const offX = -8 * s;
    const offY = -24 * s;
    ...
    // Direction flip
    container.scale.x = char.last_direction === 'left' ? -1 : 1;
  },

  getCustomizationColors(c: AvatarCustomization) {
    return {
