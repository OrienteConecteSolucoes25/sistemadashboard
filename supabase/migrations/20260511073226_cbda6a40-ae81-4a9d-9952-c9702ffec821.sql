...
-- ============================================================
-- CREA — Nova aba ART (Obras / ARTs simplificadas)
-- ============================================================

create table if not exists public.crea_art_obras (
  id uuid primary key default gen_random_uuid(),
  company_id uuid not null,
  obra text not null,
  tipo_obra text not null default 'civil',
...
create index if not exists idx_crea_art_obras_company on public.crea_art_obras(company_id, is_deleted);
create index if not exists idx_crea_art_obras_status on public.crea_art_obras(company_id, status);
create index if not exists idx_crea_art_obras_uf on public.crea_art_obras(uf);

alter table public.crea_art_obras enable row level security;

drop policy if exists "crea_art_obras_select" on public.crea_art_obras;
create policy "crea_art_obras_select" on public.crea_art_obras
...
drop policy if exists "crea_art_obras_insert" on public.crea_art_obras;
create policy "crea_art_obras_insert" on public.crea_art_obras
  for insert to authenticated
  with check (public.crea_can(auth.uid(), company_id, 'create'));

drop policy if exists "crea_art_obras_update" on public.crea_art_obras;
create policy "crea_art_obras_update" on public.crea_art_obras
  for update to authenticated
  using (public.crea_can(auth.uid(), company_id, 'edit'))
  with check (public.crea_can(auth.uid(), company_id, 'edit'));

drop policy if exists "crea_art_obras_delete" on public.crea_art_obras;
create policy "crea_art_obras_delete" on public.crea_art_obras
  for delete to authenticated
  using (public.crea_can(auth.uid(), company_id, 'delete'));

drop trigger if exists trg_crea_art_obras_upd on public.crea_art_obras;
create trigger trg_crea_art_obras_upd
  before update on public.crea_art_obras
  for each row execute function public.update_updated_at_column();

-- Adiciona crea_art_obras à allowlist de soft-delete
